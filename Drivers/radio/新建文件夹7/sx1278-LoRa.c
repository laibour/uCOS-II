...
double SX1278ALoRaReadRssi( void )
{
    ...
    {
        return B_RssiOffsetLF[LoRaSettings.SignalBw] + ( double )SX1278ALR->RegRssiValue;
    }
    else
    {
        return B_RssiOffsetHF[LoRaSettings.SignalBw] + ( double )SX1278ALR->RegRssiValue;
    }
}
...
uint32_t SX1278ALoRaProcess( void )
{
    ...
    case RFLR_STATE_RX_INIT:
        ...
        if( LoRaSettings.FreqHopOn == true )
        {
            ...
            SX1278ALoRaSetRFFrequency( B_HoppingFrequencies[SX1278ALR->RegHopChannel & RFLR_HOPCHANNEL_CHANNEL_MASK] );
        }
        else
        {
    ...
    case RFLR_STATE_RX_DONE:
        ...
        {    
            ...
            {
                RxPacketRssiValue = NOISE_ABSOLUTE_ZERO + 10.0 * B_SignalBwLog[LoRaSettings.SignalBw] + NOISE_FIGURE_LF + ( double )RxPacketSnrEstimate;
            }
            else
            {    
                SX1278ARead( REG_LR_PKTRSSIVALUE, &SX1278ALR->RegPktRssiValue );
                RxPacketRssiValue = B_RssiOffsetLF[LoRaSettings.SignalBw] + ( double )SX1278ALR->RegPktRssiValue;
            }
        }
        else                                        // HF
        {    
            if( RxPacketSnrEstimate < 0 )
            {
                RxPacketRssiValue = NOISE_ABSOLUTE_ZERO + 10.0 * B_SignalBwLog[LoRaSettings.SignalBw] + NOISE_FIGURE_HF + ( double )RxPacketSnrEstimate;
            }
            else
            {    
                SX1278ARead( REG_LR_PKTRSSIVALUE, &SX1278ALR->RegPktRssiValue );
                RxPacketRssiValue = B_RssiOffsetHF[LoRaSettings.SignalBw] + ( double )SX1278ALR->RegPktRssiValue;
            }
        }
        ...
        else // Rx continuous mode
        {
            ...
            else
            {
				...
				#ifdef __DEBUG
				DebugSendData(RFBuffer, RxPacketSize);		// 侦测接收到的数据
				#endif
			}
        }
